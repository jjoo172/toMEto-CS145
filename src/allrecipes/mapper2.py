...
def writeToFile():
  with open(utils.MAPPER2_OUT, 'w') as f:
    for d in mapping:
      f.write('%s\t%s\n' % (d, mapping[d]))
...
def _clean(a):
  ...
  quantities = ['cup', 'dash', 'drop', 'gallon', 'liter', 'ounce', 'pound', 'pinch', 'pint', 'quart', 'shot', 'spoon', 'can', 'jar', 'tub', 'box', 'fluid', 'package']
...
def importall():
  global graph, degree, recipes, mapping, invmapping, top, map2
...
def test6():
  for k in degree:
    if degree[k] >= MIN_DEGREE and k not in top:
      print k, degree[k]
